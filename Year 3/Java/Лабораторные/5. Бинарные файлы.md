...
primes (set) = []
для каждой пары (current, other) в unique:
	если НОД(current, other) != 1 && НОД(current, other) != current: простое
...
для каждой пары (current, other) в unique:
	...
	other = 2:
		...
		НОД == 1 && НОД == current => всё ещё простое, is_prime не меняется
	other = 3:
...
для каждой пары (current, other) в unique:
	...
	other = 2: 
		...
		НОД == 2 && НОД != current => не взаимнопростое, is_prime = false, выходим из цикла
...
для каждой пары (current, other) в unique:
	...
	other = 3:
		...
		НОД == 3 && НОД != current => не взаимнопростое, is_prime == false, выходим из цикла
	так как is_prime == true:
		пропускаем, 8 - не взаимнопростое
```
# Результат
