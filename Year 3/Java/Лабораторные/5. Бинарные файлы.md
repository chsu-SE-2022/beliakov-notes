...
```
# Алгоритм
1. Считываем число из файла (`current`)
2. Заносим его в список чисел
3. Для `current` и всего списка предыдущих чисел считаем наибольший общий делитель
4. Если НОД не 1, вносим в список взаимнопростых (`mutualPrimes`)
...
```
current = 2
добавляем 2 в unique 
unique (set) = [2]
primes (set) = []
для каждого (other) в unique:
	is_prime = true
	other == 2:
		current == other => пропускаем
	если is_prime:
		добавляем в primes current => primes = [2]
		
```
## 3
```
current = 3
добавляем 3 в unique 
unique (set) = [2, 3]
primes (set) = [2]
для каждого (other) в unique:
	is_prime = true
	other = 2:
		current != other => считаем НОД(3, 2) 
		НОД == 1 => всё ещё простое, is_prime не меняется
	other = 3:
		 current == other => пропускаем
	так как is_prime == true:
		добавляем 3 в primes => primes = [2, 3]
```
## 4
```
current = 4
добавляем 4 в unique 
unique (set) = [2, 3, 4]
primes (set) = [2, 3]
для каждого (other) в unique:
	is_prime = true
	other = 2: 
		current != other => считаем НОД(4, 2) 
		НОД == 2 => не взаимнопростое, is_prime = false, выходим из цикла
	так как is_prime == false:
		пропускаем, 4 - невзаимнопростое
```
## 5
```
current = 5
добавляем 5 в unique 
unique (set) = [2, 3, 4, 5]
primes (set) = [2, 3]
для каждого (other) в unique:
	is_prime = true
	other = 2: 
		current != other => считаем НОД(5, 2) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 3:
		current != other => считаем НОД(5, 3) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 4:
		current != other => считаем НОД(5, 4) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 5:
		current == other => пропускаем
	так как is_prime == true:
		добавляем 5 в primes => primes = [2, 3, 5]
```
## 6
```
current = 6
добавляем 6 в unique 
unique (set) = [2, 3, 4, 5, 6]
primes (set) = [2, 3, 5]
для каждого (other) в unique:
	is_prime = true
	other = 2: 
		current != other => считаем НОД(6, 2) 
		НОД == 2 => не взаимнопростое, is_prime = false, выходим из цикла
	так как is_prime == false:
		пропускаем, 6 - не взаимнопростое
```
## 7
```
current = 7
добавляем 7 в unique 
unique (set) = [2, 3, 4, 5, 6, 7]
primes (set) = [2, 3, 5]
для каждого (other) в unique:
	is_prime = true
	other = 2: 
		current != other => считаем НОД(7, 2) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 3:
		current != other => считаем НОД(7, 3) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 4:
		current != other => считаем НОД(7, 4) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 5:
		current != other => считаем НОД(7, 5) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 6:
		current != other => считаем НОД(7, 6) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 7:
		current == other => пропускаем
	так как is_prime == true:
		добавляем 7 в primes => primes = [2, 3, 5, 7]
```
## 8
```
current = 8
добавляем 8 в unique 
unique (set) = [2, 3, 4, 5, 6, 7, 8]
primes (set) = [2, 3, 5, 7]
для каждого (other) в unique:
	is_prime = true
	other = 2: 
		current != other => считаем НОД(8, 2) 
		НОД == 2 => не взаимнопростое, is_prime = false, выходим из цикла
	так как is_prime == false:
		пропускаем, 8 - не взаимнопростое
```
## 9
```
current = 9
добавляем 9 в unique 
unique (set) = [2, 3, 4, 5, 6, 7, 8, 9]
primes (set) = [2, 3, 5, 7]
для каждого (other) в unique:
	is_prime = true
	other = 2: 
		current != other => считаем НОД(9, 2) 
		НОД == 1 => взаимнопростое, is_prime не меняется
	other = 3:
		current != other => считаем НОД(9, 3) 
		НОД == 3 => не взаимнопростое, is_prime == false, выходим из цикла
	так как is_prime == true:
		пропускаем, 8 - не взаимнопростое
```
# Результат
